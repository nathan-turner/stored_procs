...
-- this is for the Midlevel Ref form (unused) and Ref Billing form.
-- HEADER
...
declare ref_cur cursor for
 select ctct_name, left(concat(ctct_title,'/',ctct_reserved2),63), ctct_addr_c,
  ctct_st_code, ctct_phone, ctct_hphone from
   lstcontacts where ctct_backref =  phid and ctct_type = 16;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET goon = FALSE;
-- 1. fill the header
--  rname,  ctrno,  spec,  cname,  ccity,  cst
select  ctr_cli_id,  ctr_recruiter,  ctr_no,  concat(RTRIM(at_name), ' (', at_abbr, ')')
	into  cliid,  ctrreq,  ctrno,  spec0
	from  allcontracts join  dctalliedtypes on ctr_nu_type = at_code where ctr_id =  ctrid;
...
select case when an_sex = 0 then 'Female' else 'Male' end, an_ctct_id into  psex, worka
from  lstalliednurses where an_id =  phid;
